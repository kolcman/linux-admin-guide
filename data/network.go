...
var NetworkSection = Section{
	...
	Items: []Item{
		...
		{Type: TypeKey, Key: "RTT", Desc: "ping: время туда-обратно (мс)"},
		{Type: TypeKey, Key: "TTL", Desc: "traceroute: на каждом хопе TTL+1, пока не дойдёт"},
		...
		{Type: TypeHeader, Value: "🏷️ IP-адреса"},
		{Type: TypeTip, Value: "Имя (yandex.ru) → DNS → IP (5.255.255.77). ping/traceroute ходят по IP."},
		{Type: TypeKey, Key: "10.0.0.0/8", Desc: "частные: 10.0.0.0–10.255.255.255"},
		{Type: TypeKey, Key: "172.16.0.0/12", Desc: "частные: 172.16.0.0–172.31.255.255"},
		{Type: TypeKey, Key: "192.168.0.0/16", Desc: "частные: 192.168.0.0–192.168.255.255"},
		{Type: TypeKey, Key: "100.64.0.0/10", Desc: "CGNAT: 100.64.0.0–100.127.255.255"},
		{Type: TypeKey, Key: "127.0.0.0/8", Desc: "localhost: 127.0.0.1–127.255.255.255"},
		{Type: TypeCmd, Value: "ping -c 1 127.0.0.1", Desc: "петля на себя"},

		{Type: TypeHeader, Value: "🔤 DNS"},
		{Type: TypeTip, Value: "Иерархия: . (root) → TLD (.ru .com) → 2-й уровень (yandex.ru) → 3-й (maps.yandex.ru)."},
		{Type: TypeKey, Key: "TLD", Desc: "верхний уровень: .com .ru .org .net .edu .gov …"},
		{Type: TypeKey, Key: "2-й уровень", Desc: "yandex.ru"},
		{Type: TypeKey, Key: "3-й уровень", Desc: "maps.yandex.ru (поддомен)"},
