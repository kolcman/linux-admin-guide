...
var ProcessesSection = Section{
	...
	Items: []Item{
		...
		{Type: TypeHeader, Value: "🌙 Фон (jobs)"},
		{Type: TypeTip, Value: "После & шелл печатает [N] PID. N — номер задачи в этой консоли, не PID.\njobs: Running → Done. Ctrl+Z — Stopped, потом bg N."},
		{Type: TypeKey, Key: "command &", Desc: "сразу в фон, консоль свободна"},
		{Type: TypeKey, Key: "Ctrl+Z", Desc: "остановить текущую задачу (Stopped)"},
		{Type: TypeKey, Key: "jobs", Desc: "фоновые задачи этой сессии"},
		{Type: TypeKey, Key: "bg N", Desc: "Stopped → снова Running в фоне"},
		{Type: TypeKey, Key: "fg N", Desc: "вернуть задачу на передний план (займёт консоль)"},
		{Type: TypeCmd, Value: "ping -c 1000 ya.ru > ping.txt &", Desc: "ping в фон, вывод в файл; в консоли [N] PID"},
		{Type: TypeCmd, Value: "jobs", Desc: "список задач"},
		{Type: TypeCmd, Value: "bg 1", Desc: "продолжить задачу 1 в фоне"},
		{Type: TypeCmd, Value: "fg 1", Desc: "вернуть задачу 1 в консоль"},
		{Type: TypeCmd, Value: "host ya.ru &>/dev/null; echo $?", Desc: "только код возврата, без вывода (есть в /etc/hosts — ок без DNS)"},
